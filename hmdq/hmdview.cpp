...
{
    double l = raw["tan_left"].get<double>() * norm;
    double r = raw["tan_right"].get<double>() * norm;
    double b = raw["tan_bottom"].get<double>() * norm;
    double t = raw["tan_top"].get<double>() * norm;
    harray2d_t res = {{l, b}, {r, b}, {r, t}, {l, t}};
    return res;
}
...
json get_fov(const json& raw, const json& mesh, const harray2d_t* rot)
{
    ...
    if (!mesh.is_null()) {
        ...
        harray2d_t uvverts = mesh["verts_opt"];
    ...
    hface_t lrbt_face = {len - 4, len - 3, len - 2, len - 1};
    ...
    if (nullptr != rot) {
        ...
        auto tverts3d = matmul(verts3d, xt::transpose(*rot));
        // get the last column and use it to normalize the points
        auto tcol = xt::view(tverts3d, xt::all(), 2);
        auto zcol = xt::expand_dims(tcol, 1);
    ...
    hvector_t base = {0, 0, -1};
    for (size_t i = 0, e = pts3d.shape(0); i < e; ++i) {
        auto p = xt::view(pts3d, i);
...
{
    // horizontal FOV
    auto fov_hor = fov_head[REYE]["deg_right"].get<double>()
        - fov_head[LEYE]["deg_left"].get<double>();

    // vertical FOV calculated from "straight ahead" look
    auto fov_ver = std::min(fov_head[REYE]["deg_top"].get<double>(),
    ...
    hvector_t left_bottom = fov_head[LEYE]["fov_pts"][0];
    hvector_t left_top = fov_head[LEYE]["fov_pts"][6];
    hvector_t right_top = fov_head[REYE]["fov_pts"][4];
    hvector_t right_bottom = fov_head[REYE]["fov_pts"][2];
    auto diag1 = angle_deg(left_bottom, right_top);
    auto diag2 = angle_deg(left_top, right_bottom);
    auto fov_diag = (diag1 + diag2) / 2;

    // overlap
    auto overlap = fov_head[LEYE]["deg_right"].get<double>()
...
{
    harray2d_t left = e2h[LEYE];
    harray2d_t right = e2h[REYE];
    auto cols = left.shape(1);
